...
describe('Projects API Integration Tests', () => {
  let app: import('h3').NodeListener;
  ...
  const postProject = (data: {
    ...
    let req = request(app).post('/api/projects');
    if (data.prompt !== undefined) {
      req = req.field('prompt', data.prompt);
    }
    if (data.panelCount !== undefined) {
      req = req.field('panelCount', String(data.panelCount));
    }
    return req;
  };
  ...
  describe('POST /api/projects', () => {
    it('should create a project and return 201 Created', async () => {
      ...
      expect(response.body.data).toHaveProperty('status', 'pending_creation');
    });
